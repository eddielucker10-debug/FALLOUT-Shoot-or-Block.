 ...
 */
export const FALLING_TYPES: FallingType[] = NAMES.map(
  ([name, shape, preferred, desc], i) => {
    ...
    const splits = /spore|toxic|slime|fungal|venom|cell|blob|cluster/i.test(
      name,
    )
    const volatile =
      preferred === "block" &&
      /magma|doom|singularity|eclipse|terror|dread|cursed|core|meteor/i.test(
        name,
      )
    return {
      id: i,
